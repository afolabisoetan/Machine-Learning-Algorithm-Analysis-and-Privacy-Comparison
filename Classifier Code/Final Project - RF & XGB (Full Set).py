...
pd.DataFrame({'Iteration': np.arange(1, 31), 'Full_Dataset_Recall': rf_recall})\
   .to_csv('rf_fraud_detection_results_FULL.csv', index=False)


pd.DataFrame({'Iteration': np.arange(1, 31), 'Full_Dataset_Recall': xgb_recall})\
   .to_csv('xgb_fraud_detection_results_FULL.csv', index=False)
